...
function inizializza() {
	...
	$('#home_slides, #popup_slides, .header_popup').superslides({
	...
	$('#news_slides, #box_slides').superslides({
...
function transizioni() {
	...
	$(document).on("click tap", ".prenotazione", function(e) { // Al click su prenota
		...
		$.post('http://www.acantomilano.com/beta/include/prenota_popup.php', { 
...
function validaInviaForm() {
    ...
    $("#invia_prenota").on("click tap", function(e) { // All'invio del form
        ...
        if ($("#prenota_form input[required], #prenota_form textarea[required]").val().length === 0) { // Se i campi richiesti risultano ancora vuoti
            ...
            $.post("http://www.acantomilano.com/beta/include/form_prenota.php", { 
...
function disponibile() {
	...
	$(".disponibilita").each(function() { // Per ogni elemento
		...
		if (($(".dispo_icona", this).hasClass("disponibile_ico")) && ($(".dispo_label", this).hasClass("disponibile"))) { // Se l'icona risulta attiva - L'evento è disponibile
			...
			$(".dispo_label", this).html("Available"); // Segna come disponibile

		} else { 

			$(".dispo_icona", this).removeClass("disponibile_ico"); // Allora rendila attiva
			$(".dispo_label", this).html("Not Available"); // Segna come disponibile
